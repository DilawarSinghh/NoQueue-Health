  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const [profRes, agentRes] = await Promise.all([
        ...
        supabase
          .from("agent_profiles")
          .select("department, experience_years, bio, whatsapp_number, rating, rating_count")
          .eq("user_id", user.id)
          .maybeSingle(),
      ]);
      ...
      if (agentRes.data) {
        ...
        setRating(agentRes.data.rating ?? null);
        setRatingCount(agentRes.data.rating_count ?? 0);
      }
      setLoading(false);
    });
      ...
      >
        ...
        <GlassCard className="p-6 sm:p-8">
          ...
          <form onSubmit={handleSave} className="mt-6 space-y-5" noValidate>
            ...
            {/* Rating — read-only, computed from submitted ratings */}
            {ratingCount > 0 && (
              <div className="grid gap-2">
                <Label>Your rating</Label>
                <div className="flex h-11 items-center gap-3 rounded-xl border border-input bg-muted/40 px-3 text-sm">
                  <div className="flex items-center gap-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <svg
                        key={i}
                        className={`h-4 w-4 ${i < Math.round(rating ?? 0) ? "fill-amber-400 text-amber-400" : "text-muted-foreground/30"}`}
                        viewBox="0 0 20 20"
                        fill="currentColor"
                        aria-hidden="true"
                      >
                  ...
                  <span className="ml-auto rounded-full bg-muted px-2 py-0.5 text-xs">read-only</span>
                </div>
              </div>
            )}
